...
class App {
     ...
     * this window of the last successful load is
     * short-circuited to a no-op. Tuned to 500ms —
     * a human double-click on the load button takes
     * ~200-300ms (faster than a deliberate second
     * click), and the round-50 rehydration + setTimeout
     * schedule typically completes within ~50ms. 500ms
    ...
    /**
     * Round 106 — debounce window for `saveGame`
     * in milliseconds. Same 500ms tuning as
     * `LOAD_DEBOUNCE_MS` — see round-104 JSDoc
     * for the rationale (human double-click
     * ~200-300ms + round-50 telemetry gate
     * ~50ms + margin). A future round-107+
     * could expose both constants as a single
     * debug knob in the help overlay
     * (`settings.actionDebounceMs`).
     */
    private static readonly SAVE_DEBOUNCE_MS = 500;
    /**
     * Round 107 — debounce window for
     * `rollWorldEvent` in milliseconds. Same
     * 500ms tuning as `LOAD_DEBOUNCE_MS` and
     * `SAVE_DEBOUNCE_MS` (human-double-click
     * ~200-300ms + telemetry/visual margin).
    ...
    /**
     * Round 109 — debounce window for
     * `enterAtom` (round-65 keyboard 1-8
     * jump path) in milliseconds. Same
     * 500ms tuning as the other 3 debounced
     * actions (human-double-click
     * ~200-300ms + render/margin). The
     * 4th `ActionDebouncer` use case —
     * demonstrates the round-108 helper
     * class works for new code. A future
     * round-110+ could fold all 4 into a
     * single `ACTION_DEBOUNCE_MS` constant
     * + update the test casts to use
     * `debouncer.windowSizeMs`.
     */
    private static readonly ENTER_ATOM_DEBOUNCE_MS = 500;
