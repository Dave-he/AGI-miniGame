...
use crate::atom::{AtomContext, AtomId, AtomRegistry, AtomRunner};
use crate::ai_engine::DimensionBlueprint;
use crate::world_state::UnifiedWorldState;
...
impl Dimension {
    ...
    pub fn complete(&mut self, ctx: &mut AtomContext) {
        ...
        data.insert("time".to_string(), Value::Float(self.elapsed_time));
...
impl DimensionRunner {
    ...
    pub fn start_dimension(&mut self, config: DimensionConfig) -> bool {
        ...
        let mut ctx_mut = ctx;
        dimension.start(&mut ctx_mut);

        self.active_dimension = Some(dimension);
        true
    }
...
mod tests {
    ...
    impl MockAtom {
        fn new(id: &str) -> Self {
            Self {
                id: id.to_string(),
                phase: crate::atom::AtomPhase::Uninitialized,
            }
        }
    }
    ...
    impl Atom for MockAtom {
        ...
        fn atom_name(&self) -> &str { "Mock" }
        fn on_init(&mut self, _ctx: &mut AtomContext) { self.phase = crate::atom::AtomPhase::Initialized; }
        fn on_enter(&mut self, _ctx: &mut AtomContext) { self.phase = crate::atom::AtomPhase::Running; }
        fn on_update(&mut self, _ctx: &mut AtomContext) {}
        fn on_pause(&mut self, _ctx: &mut AtomContext) { self.phase = crate::atom::AtomPhase::Paused; }
        fn on_resume(&mut self, _ctx: &mut AtomContext) { self.phase = crate::atom::AtomPhase::Running; }
        fn on_exit(&mut self, _ctx: &mut AtomContext) { self.phase = crate::atom::AtomPhase::Completed; }
        fn on_destroy(&mut self) { self.phase = crate::atom::AtomPhase::Uninitialized; }
        ...
        fn current_phase(&self) -> crate::atom::AtomPhase { self.phase }
    ...
    fn make_registry() -> AtomRegistry {
        ...
        for id in &["match3", "tower_defense"] {
